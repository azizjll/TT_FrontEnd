...
export class HomeAdminComponent implements OnInit {
  ...
  saveParent() {
    ...
    const utilise = Number(this.parentForm.utilise ?? 0);
      const email     = this.parentForm.email ?? '';   // ✅ ajout
  ...
  loadCampagnesPuisCandidatures(): void {
    this.campagneService.getAllCampagnes().subscribe({
      ...
      if (campagneActive) {
         this.campagneId = campagneActive.id;
...
if (campagneMemeAnnee) {
      ...
      next: () => {
        if (this.selectedStructureId &&
          ...
          this.affectationService.affecterSaisonnier(
            ...
            next: () => {
              alert('Candidature et structure mises à jour ✅');
              this.loadCandidatures();
              this.closeDossier();
            },
            error: () => {
              alert('Candidature sauvegardée mais erreur sur la structure ⚠️');
              this.loadCandidatures();
              this.closeDossier();
            }
          });
  ...
  this.documentService.getDocumentByType('CIRCULAIRE_2025').subscribe({
    next: (doc) => {
      if (doc?.url) {
        ...
        // ✅ bypass justifié : URL validée contre les origines du backend
  ...
  this.documentService.uploadDocument(file, 'CIRCULAIRE_2025').subscribe({
    ...
    error: (err) => {
      console.error('Erreur upload PDF :', err);  // ⚠️ À remplacer par log si possible
  ...
  private getDureeContratPour(moisTravail?: string): number {
  return moisTravail === 'JUILLET_AOUT' ? 60 : this.presenceConfig.dureeContrat;
}
  loadPresenceRows(): void {
    ...
    this.presenceRows = candidaturesFiltrees.map(c => ({
      ...
      moisTravail: c.saisonnier.moisTravail   // ← ajouté
    }));
  ...
  this.presenceRows = this.presenceRows.map(row => {
    const dureeContrat = this.getDureeContratPour(row.moisTravail);
    return {
      ...row,
      dureeContrat,
